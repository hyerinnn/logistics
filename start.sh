...
docker exec cli  peer chaincode install -n histcc -v 1.0 -l golang -p github.com/chaincode/histcc

### 생활물류용 체인코드
docker exec cli  peer chaincode install -n shippcc -v 1.0 -l golang -p github.com/chaincode/shippcc
...
docker exec cli  peer chaincode instantiate -o orderer.orderer.com:7050 --tls true --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/orderer.com/orderers/orderer.orderer.com/msp/tlscacerts/tlsca.orderer.com-cert.pem -C b2bchannel -n histcc -v 1.0 -c '{"Args":[""]}'
